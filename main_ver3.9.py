...
gregorianMonthNameArray = ["ינואר", "פברואר", "מרץ", "אפריל", "מאי", "יוני", "יולי", "אוגוסט", "ספטמבר", "אוקטובר",
                           "נובמבר", "דצמבר"]
...
hebrewMonthArray = ["Nisan", "Iyyar", "Sivan", "Tamuz", "Av", "Elul", "Tishrei", "Cheshvan", "Kislev", "Tevet", "Shvat",
                    "Adar1", "Adar2"]
...
def findDates(str):
    ...
    gregorianDayMonth = "(?:" + makeOrExp(reversed(range(1, 32)),
                                          gregorianDaysFormats) + makeSeparatorsExp() + makeOrExp(
    ...
    gregorianMonthDay = "(?:" + makeOrExp(reversed(range(1, 13)),
                                          gregorianMonthFormats) + makeSeparatorsExp() + makeOrExp(
...
def addDateTagToNodeFromText(node, dateArr):
    ...
    for i in range(len(dateArr) - 1):
        ...
        element = ET.Element('date', attrib={'eId': idGen(), 'date': date})
    ...
    if len(re.findall(makeOrExp(hebrewMonthArray, hebrewMonthFormats), dateArr[i])) > 0:
        ...
        date = getGregorianDateAttribute(dateArr[i])
    element = Element('date', attrib={'eId': idGen(), 'date': date})
    element.text = dateArr[i]
    tempStr = tempStr[tempStr.index(dateArr[i]) + len(dateArr[i]):]
...
def main():
    path = easygui.fileopenbox(msg="Choose a xml file", filetypes=["*.xml", "*.xml"])
